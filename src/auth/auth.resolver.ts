...
export class AuthResolver {
    ...
    @Mutation(() => Auth)
    async authBuild( @Args('auth') auth: CreateAuthInput ): Promise<Auth> {
        console.log('mutation authBuild() is running...');
        return await this.authService.build(auth);
    }


    @Query(() => LoginToken)
    async authLogin( @Args('loginAuthInput') loginAuthInput: LoginAuthInput ): Promise<LoginToken> {
