...
export class MerchantResolver {
    ...
    async merchantBuild( @Args('merchant') merchant: BuildMerchantInput ): Promise<Merchant> {
        console.log('mutation build() is running...');
        return await this.merchantService.build(merchant);
    }
    @Mutation(type => Merchant)
    async merchantRebuild( @Args('merchant') merchant: UpdateMerchantInput ): Promise<Merchant> {
        console.log('mutation rebuild() is running...');
        return await this.merchantService.rebuild(merchant);
    }
    @Query(type => Merchant)
    async merchantFetchById (@Args('merchant_id') merchant_id: number): Promise<Merchant> {
        console.log('Query merchantetchById() is running...');
        ...
        console.log('Query merchantetchById() merchant> ', merchant);
        return merchant;
    }
